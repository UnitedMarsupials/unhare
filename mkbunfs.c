...
/*
 * Write the bundle as a C array placed in its own ".bun" section.  Mach-O
 * section attributes name both a segment and a section; ELF names only the
 * section.
 */
static void
emit_source(const char *path, const uint8_t *data, size_t len)
{
	...
	fprintf(f, "__attribute__((__section__(\"__DATA,.bun\"), "
	    "__used__))\n");
#else
